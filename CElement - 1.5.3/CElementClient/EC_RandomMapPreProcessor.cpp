...
bool CECTerrainPreprocessor::LoadFile(const char* szMapName){
	...
	try	{
		...
		for (int i=0;i<fileHeader.iNumTexture;i++)	{
			int len = 0;
...
void CECRandomMapProcess::ProcessRepeatSubTern(){
	...
	for (unsigned int i=0;i<(int)newSub.size();i++){
...
void CECRandomMapProcess::GenerateImage(abase::vector<A3DSurface*>& srcImgVec, const AString& path, const AString& fileName){
	...
	int nCols = int(sqrt((float)srcImgVec.size()));
	int W = W0 * nCols;
/*	n = 1;
	while( n < W ) n *= 2;
	W = n;
...
/*	n = 1;
	...
	*/
	...
	A3DDevice* device = g_pGame->GetA3DDevice();
	pDestSurface->Create(device, W, H, nFormat);
	...
	for( i = 0; i < nRows; i++ ){
		for( j = 0; j < nCols; j++ ){
			...
			if( nIndex < (int)srcImgVec.size() ){
				...
				a_pt[0].x = W0 * j;
				a_pt[0].y = H0 * i;
				device->UpdateSurface(srcImgVec[nIndex]->GetDXSurface(), a_rc, pDestSurface->GetDXSurface(), a_pt);
...
//	bool bval = pMan->LoadSurfaceFromFile(mapfileName, 0, &pTargetSurface);
	...
	A3DDevice* device = g_pGame->GetA3DDevice();
	pDestSurface->Create(device, W + W_LEFT + W_LEFT, H + H_TOP + H_TOP, nFormat);
	...
	a_pt[0].x = W_LEFT;
	a_pt[0].y = H_TOP;
	bool val = device->UpdateSurface(pTargetSurface->GetDXSurface(), a_rc, pDestSurface->GetDXSurface(), a_pt);
	...
	a_pt[0].x = 0;
	a_pt[0].y = H_TOP;
	val = device->UpdateSurface(pBorder[0]->GetDXSurface(), a_rc_left, pDestSurface->GetDXSurface(), a_pt);
	...
	a_pt[0].x = 0;
	a_pt[0].y = 0;
	val = device->UpdateSurface(pBorder[1]->GetDXSurface(), a_rc_top, pDestSurface->GetDXSurface(), a_pt);
	...
	a_pt[0].y = H_TOP;
	val = device->UpdateSurface(pBorder[2]->GetDXSurface(), a_rc_left, pDestSurface->GetDXSurface(), a_pt);
	...
	val = device->UpdateSurface(pBorder[3]->GetDXSurface(), a_rc_top, pDestSurface->GetDXSurface(), a_pt);
...
void CECRandomMapFogMask::UpdateByPos(const A3DVECTOR3& pos){
	...
	for (int r=r1;r<r2;r++) {
		...
		for (int c=c1;c<c2;c++)	{
			int tr = fabs((float)r - y);
			int tc = fabs((float)c - x);
