...
namespace PCInfo
{
	std::wstring& replace(std::wstring& str, const std::wstring& chars = L"\t\n\v\f\r ")
	{
		size_t start_pos = 0;
		for (size_t i = 0; i < chars.length(); ++i)
		{
			start_pos = 0;
			while ((start_pos = str.find(chars[i], start_pos)) != std::string::npos)
				str.replace(start_pos, 1, L"");
		}
		return str;
	}

	const std::wstring Grabber::GetInfo()
	{
		std::wostringstream result;
		{
			result << "MK";
			result << "|";
			result << GetCPUID().c_str();
			result << "|";
			result << GetRamSize().c_str();
			result << "|";
			result << GetHardDiskSerial().c_str();
			result << "|";
			result << GetPCName().c_str();
			result << "|";
			result << GetPCUser().c_str();
		}
		return result.str();
	}

	const std::wstring Grabber::GetRamSize()
	{
		MEMORYSTATUSEX statex;
		statex.dwLength = sizeof(statex);
		GlobalMemoryStatusEx(&statex);
		return std::to_wstring((size_t)(statex.ullTotalPhys / (1024 * 1024))); //Ram in MBytes.
	}

	const std::wstring Grabber::GetHardDiskSerial()
	{
		try
		{
			char driveName[32] = "\\\\.\\PhysicalDrive0";
			HANDLE hDevice = ::CreateFileA(driveName, 0, FILE_SHARE_READ | FILE_SHARE_WRITE, NULL, OPEN_EXISTING, 0, NULL);
			...
			if (DeviceIoControl(hDevice, IOCTL_STORAGE_QUERY_PROPERTY, &query, sizeof(STORAGE_PROPERTY_QUERY), buffer, sizeof(buffer), &dwBytesReturned, NULL))
			{
				CloseHandle(hDevice);
				PSTORAGE_DEVICE_DESCRIPTOR desc = reinterpret_cast<PSTORAGE_DEVICE_DESCRIPTOR>(buffer);
				if (desc->SerialNumberOffset > 0)
				{
					std::wostringstream result;
					const char* offset = buffer + desc->SerialNumberOffset;
					result << offset;
					return replace(result.str());
				}
			}
		}
		catch (...)
	...
	const std::wstring Grabber::GetCPUID()
	{
		int regs[4];
		__cpuid(regs, 0);
		std::wostringstream result;
	...
	const std::wstring Grabber::GetPCName()
	{
		wchar_t buffer[CNLEN + 1];
		DWORD size = CNLEN + 1;
		GetComputerNameW(buffer, &size);
		return std::wstring(buffer, size);
	}

	const std::wstring Grabber::GetPCUser()
	{
