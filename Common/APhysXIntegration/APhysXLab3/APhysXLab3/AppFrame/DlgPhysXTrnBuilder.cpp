...
bool CDlgPhysXTrnBuilder::BuildSubTerrainPhysXData(int r, int c, int iSubIndex)
{
	...
	A3DTerrain2Loader* pLoader = m_pTerrain->GetTerrainLoader();
	for (i=rc.top; i < rc.bottom; i++)
	{
		for (j=rc.left; j < rc.right; j++)
		{
			A3DTerrain2Block* pBlock = 0;
#ifdef _ANGELICA3
			pBlock = pLoader->LoadBlock(0, i, j, 0);
#else
			pBlock = pActArea->GetBlock(i, j, false);
#endif
			ASSERT(pBlock);
...
bool CDlgPhysXTrnBuilder::SaveSubTerrainPhysXData(int iSubIndex, float fMinHei, float fMaxHei)
{
	...
	data.mPatchColumnNum	= 128;
	data.mPatchRowNum		= 128;
