...
class BrushcharCtrler : IDynObjStateReceiver
{
...
public:
	...
	CDR_INFO& GetCDRInfo() { return m_CDRInfo; }
	...
	virtual void OnRegisterDynBrushObj(CBrushesObj* pObj);
	virtual void OnUnRegisterDynBrushObj(CBrushesObj* pObj);

	float GetDistToGround() const;

	A3DVECTOR3 GetVelocity() const;
...
protected:
	...
	float m_fDistToGround;//distance to ground
	...
	struct InDynamic{//在动态物体坐标系下的描述，用来实现推动效果
		A3DMATRIX4 invmat;//coordinate
		A3DVECTOR3 vP;//capsule position
		CBrushesObj* pBrushObj;//dynamic object
	};
