...
CTerrain::CTerrain(const CTerrain& rhs) : m_nNumVertX(rhs.m_nNumVertX),m_nNumVertZ(rhs.m_nNumVertZ),m_ox(rhs.m_ox),m_oz(rhs.m_oz),m_vGridSize(rhs.m_vGridSize),m_vGridSizeInv(rhs.m_vGridSizeInv),m_config(rhs.m_config)
{
	int total = m_nNumVertZ * m_nNumVertX;
	m_pHeights = new float[total];
	memcpy(m_pHeights,rhs.m_pHeights,total * sizeof(float));
}

CTerrain& CTerrain::operator = (const CTerrain& rhs)
{
	m_nNumVertX = rhs.m_nNumVertX;
	m_nNumVertZ = rhs.m_nNumVertZ;
	m_ox = rhs.m_ox;
	m_oz = rhs.m_oz;
	m_vGridSize = rhs.m_vGridSize;
	m_vGridSizeInv = rhs.m_vGridSizeInv;
...
bool CTerrain::GetHeightAt(float x, float z, float& y, A3DVECTOR3* pNormal) const 
{
	...
	if (v0 + m_nNumVertX + 1 >= m_nNumVertZ * m_nNumVertX)
	{
		//ASSERT(0);
		return false;
	}
	...
	if (pNormal)
	{
		A3DVECTOR3 v1(vGridSize, dh1, 0);
		A3DVECTOR3 v2(0, dh2, vGridSize);
		*pNormal = CrossProduct(v2, v1);
		pNormal->Normalize();
	}
