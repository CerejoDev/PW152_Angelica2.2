...
void ObjStateReportMgr::OnNotifyRename(const IObjBase& obj)
{
	IOSR::iterator it = m_Receivers.begin();
	IOSR::iterator itEnd = m_Receivers.end();
	for (; it != itEnd; ++it)
		(*it)->OnNotifyRename(obj);
}
...
void ApxObjOptReportMgr::OnAddChildNotify(ApxObjBase* pParent, ApxObjBase* pChild)
{
	AOOR::iterator it = m_Receivers.begin();
	AOOR::iterator itEnd = m_Receivers.end();
	for (; it != itEnd; ++it)
		(*it)->OnAddChildNotify(pParent, pChild);
}

void ApxObjOptReportMgr::OnRemoveChildNotify(ApxObjBase* pParent, ApxObjBase* pChild)
{
	AOOR::iterator it = m_Receivers.begin();
	AOOR::iterator itEnd = m_Receivers.end();
	for (; it != itEnd; ++it)
		(*it)->OnRemoveChildNotify(pParent, pChild);
}

void ApxObjOptReportMgr::RegisterReceiver(ApxObjOperationReport& aoor)
{
	ApxObjOperationReport* pAOOR = &aoor;
	...
	m_Receivers.push_back(pAOOR);
}

void ApxObjOptReportMgr::UnregisterReceiver(ApxObjOperationReport& aoor)
{
	ApxObjOperationReport* pAOOR = &aoor;
	AOOR::iterator it = m_Receivers.begin();
	AOOR::iterator itEnd = m_Receivers.end();
...
void ApxObjOptReportMgr::ClearAllReceiver()
{
	m_Receivers.clear();
}