...
bool CCoordinateDirection::GetLastRotateInfo(float& outAngleRadians, A3DVECTOR3& outLocalAxis, A3DVECTOR3& outGlobalAxis) const
{
	if (EDIT_TYPE_ROTATE != m_enumET)
		return false;

	outAngleRadians = m_LastRotateAngleRadians;
	outLocalAxis = GetSelectedAxis();
	outGlobalAxis = m_matRotate * outLocalAxis;
	outGlobalAxis.Normalize();
	return true;
}
...
//					m_matRotate = mtTmp * m_matRotate;
					...
					m_LastRotate.Identity();
					m_LastRotate.RotateAxis(vAxis, m_LastRotateAngleRadians);
					m_matRotate = m_LastRotate * m_matRotate;
				}
			}
			break;
		default:
