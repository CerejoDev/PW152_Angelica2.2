...
CPhysXObjSkinModel::CPhysXObjSkinModel(int UID)
{
	...
	m_iCSStartID = 0;
	m_pCSConfig  = 0;
	m_propSM.InitTypeIDAndHostObject(ObjManager::OBJ_TYPEID_SKINMODEL, *this, UID);
	SetProperties(m_propSM);
	SetRenderable(m_SMRender);
}
...
bool CPhysXObjSkinModel::OnLoadModel(IEngineUtility& eu, const char* szFile)
{
	...
	m_SMRender.SetModelPtr(m_pSkinModel);
	return true;
}
...
bool CPhysXObjSkinModel::SyncDataPhysXToGraphicMyself()
{
	...
	if (0 != m_pModelPhys)
	{
		...
		m_propSM.SyncPoseDataFromPhysX();
		return bRtn;
	}
...
CPhysXObjSkinModel* CPhysXObjSkinModel::NewObjAndCloneData() const
{
	assert(0 != m_pScene);
	IObjBase* pObjBase = ObjManager::GetInstance()->CreateObject(ObjManager::OBJ_TYPEID_SKINMODEL, true, *m_pScene);
	CPhysXObjSkinModel* pObj = dynamic_cast<CPhysXObjSkinModel*>(pObjBase);
	if (0 != pObj)
		pObj->m_propSM = m_propSM;
	return pObj;
}
...
bool CPhysXObjSkinModel::OnGetPose(A3DMATRIX4& matOutPose, bool& outWithScaled) const
{
	if (0 == m_pSkinModel)
		return false;
...
bool CPhysXObjSkinModel::AddForce(const NxVec3& pt, const NxVec3& dirANDmag, int iPhysForceType, float fSweepRadius)
{
	assert(0 != m_pModelPhys);
...
#ifdef _ANGELICA21
	return m_pModelPhys->AddForce(APhysXConverter::N2A_Vector3(start), A3DVECTOR3(dir.x, dir.y, dir.z), mag, 3, iPhysForceType, fSweepRadius);
#else
	return m_pModelPhys->AddForce(APhysXConverter::N2A_Vector3(start), A3DVECTOR3(dir.x, dir.y, dir.z), mag, 3, iPhysForceType);
#endif
}
