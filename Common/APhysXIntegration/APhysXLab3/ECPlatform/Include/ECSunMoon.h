...
#ifndef _ECSUNMOON_H_
#define _ECSUNMOON_H_
...
public:
	...
	bool Init(ECGraphicsModule* pGraphics);
	bool Release();

	// render and tick
	bool Render(A3DViewport * pViewport);
	bool RenderSolarFlare(A3DViewport * pViewport);
	...
	bool SetSunColor(const A3DCOLORVALUE& color);
	bool SetMoonColor(const A3DCOLORVALUE& color);

	//	Set/Get sun light's direction
	//	bOnlySetLongitude: true, only change sun's longitude and keep sun's height unchanged
	bool SetLightDir(const A3DVECTOR3& vecLightDir, bool bOnlySetLongitude=false);
	const A3DVECTOR3& GetLightDir() { return m_vecLightDir; }

	inline const A3DCOLORVALUE& GetSunColor()	{ return m_colorSun; }
	inline const A3DCOLORVALUE& GetMoonColor()	{ return m_colorMoon; }
