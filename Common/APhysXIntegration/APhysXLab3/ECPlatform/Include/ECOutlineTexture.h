...
//#define OUTLINE_TILE_WIDTH       128
//#define OUTLINE_TILE_HEIGHT       128
//#define OUTLINE_TILE_GRID_LEN     1

#define   OUTLINE_TILE_MAGIC    (DWORD)(('o'<<24)| ('l'<<16)|('k'<<8)|('w'))
#define   OUTLINE_TILE_VERSION  (DWORD)(0x00000003)
...
class ECOutlineTexture  
{
	...
	friend class ECOutlineQuadTree;
private:
...
public:
	ECOutlineTexture();
	virtual ~ECOutlineTexture();

	bool Init(ECTerrainOutline2 * pOutline, int iLevel, int iSubIndex);
	void Release(); 
	...
	void RenderReplaceTerrain(A3DTrn2RenderSlot* pSlot);

private:

	bool _LoadTex(ECTerrainOutline2 * pOutline);
};
...
{
public:
	ECOutlineTextureCache();
	virtual ~ECOutlineTextureCache();
	bool Init(ECTerrainOutline2 * pOutline);
	void Release();
	void  ActivateTile(const A3DVECTOR3& origin, float radius, DWORD dwFrame);
	

	ECOutlineTexture * FindTex(int iLevel, int iSubIndex);
	ECOutlineTexture * AddTex(int iLevel, int iSubIndex);
	void ReplaceTex(ECOutlineTexture * pTex);
	...
	void RenderReplaceTerrain(DWORD dwFrame, int iLevel, float cx, float cz, A3DTrn2RenderSlot* pSlot);

	inline int GetTexSubIndex(int iLevel, float x, float z);

	ECOutlineQuadTree * GetTile(ECOutlineTexture * pTex, int iSubIndex);
	bool                 ReleaseTile(ECOutlineTexture * pTex, int iSubIndex);
	bool                 ActivateTile(ECOutlineTexture * pTex, int iSubIndex);

	void     SmoothTex(ECOutlineTexture * pTex);
	ECOutlineTexture  * GetNeighborTex(ECOutlineTexture * pTex, DWORD dwDir);
private:
	ECOutlineTexture * m_pCache[OUTLINE_TEX_CACHE_SIZE];
	int        m_nSize;    //real size of the cache

	ECTerrainOutline2 * m_pOutline;
	
//	ECOutlineQuadTree * m_pTiles[88];
	ECOutlineQuadTree ** m_pTiles;
	...
	bool _SmoothTex(ECOutlineTexture * pTex1, DWORD dwDir, ECOutlineTexture * pTex2);
