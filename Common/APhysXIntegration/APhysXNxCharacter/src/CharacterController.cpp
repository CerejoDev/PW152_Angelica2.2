									...
									const NxExtendedBounds3& world_box)
{
	...
	for(NxU32 j=0;j<nb_boxes;j++)
	{
		if(!world_box.intersect(boxes[j]))
			continue;
		...
		UserBox->mUserData	= box_user_data[j];
		UserBox->mOffset	= Origin;
		UserBox->mBox		= boxes[j];
	}
...
//	const NxU32 MaxIterDown = gWalkExperiment ? MaxIter : 1;
	...
	if(PerformUpPass)
	{
		...
		// modified by Yang Liu 2010-01-18
		// to generate callback if auto-stepping is zero
		if (0 != StepOffset)
			mValidateCallback = false;
		// end of modify
	...
	if(PerformDownPass)
	{
		...
		// modified by Yang Liu 2011-05-03
		// fix issue: In the walk-experiment we didn't do any up motion although 'UpVector' was updated by 'StepOffset'.
		// So, we shouldn't undo it.
		if(!SideVector.isZero() && !mWalkExperiment)	// We disabled that before so we don't have to undo it in that case
