...
{
	float fHeight = 0.0f;
	if (Terrain.GetPosHeight(Pos, fHeight))
...
bool CCameraController::EntryDefBindingMode(A3DVECTOR3& outFocusPos, A3DTerrain2& Terrain, A3DViewport* pVP)
{
	...
	if (0 == pVP)
	{
		...
		Terrain.GetPosHeight(outFocusPos, outFocusPos.y);
	}
	else
	{
		A3DVECTOR3 cameraPos = m_Camera.GetPos();
		A3DVECTOR3 ZInfinite(float(m_CenterX), float(m_CenterY), 1.0f);
...
void CCameraController::MoveCameraFreeFly(float deltaTimeSec, A3DTerrain2* pTerrain)
{
	...
	if (bMoveCamera)
	{
		if (0 != pTerrain)
		{
			...
			GetPosGroundCull(vNewPos, *pTerrain);
			vDelta = vNewPos - vPos;
		}
