...
CDynProperty::CDynProperty(const CDynProperty& rhs) : IPropPhysXObjBase(rhs)
{
	m_pAC = 0;
	OnSetToEmpty();
	m_roleType = rhs.m_roleType;
	m_apType = rhs.m_apType;
	m_rbFlags = rhs.m_rbFlags;
	m_ccFlags = rhs.m_ccFlags;
}
...
bool CDynProperty::SetRoleType(const RoleType& rt)
{
	...
	{
		SetAPType(IActionPlayerBase::AP_SEQUENCE_ACTION);
		SetCCType(CAPhysXCCMgr::CC_TOTAL_EMPTY);
...
void CDynProperty::EnterRuntimeNotify(APhysXScene& aPhysXScene)
{
	CPhysXObjDynamic* pDynObj = static_cast<CPhysXObjDynamic*>(m_pHostObject);
	assert(0 != pDynObj);
	if (0 != pDynObj)
...
void CDynProperty::LeaveRuntimeNotify()
{
...
bool CDynProperty::LoadFromFile(NxStream& nxStream, bool& outIsLowVersion)
{
	...
	{
		m_roleType = RoleType(nxStream.readDword());
		m_apType  = APType(nxStream.readDword());
		m_rbFlags = nxStream.readDword();
		m_ccFlags = nxStream.readDword();
	}

	return true;
}

bool CDynProperty::OnSendToPropsGrid(CBCGPPropList& lstProp)
{
	if (!IPropPhysXObjBase::OnSendToPropsGrid(lstProp))
		return false;
...
bool CDynProperty::OnPropertyChanged(const CBCGPProp& prop)
{
...
void CDynProperty::OnUpdatePropsGrid(CBCGPPropList& lstProp)
{
