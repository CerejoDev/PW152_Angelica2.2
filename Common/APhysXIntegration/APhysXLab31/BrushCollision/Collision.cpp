...
bool ICollisionSupplier::CollideWithTerrain(const A3DVECTOR3& vStart, const A3DVECTOR3& vDelta, float& fFraction, A3DVECTOR3& vHitNormal, bool& bStart)
{
	...
	bStart = false;
	float h1 = 0.0f;
	pTerrain->GetPosHeight(vStart, h1, &vHitNormal);
	...
	int a = (nWid +1) * (nHei +1);
	int b = nTriangles * 3;
	if (!pTerrain->GetFacesOfArea(vStart, nWid, nHei, pVerts, pIndices, a, b))
	{
		delete[] pVerts;
		delete[] pIndices;
		return false;
	...
	pTerrain->GetPosHeight(vTempEnd, h1);
	...
	{	
		...
		do
		{
			vTemStart += vTempDelta;
 			pTerrain->GetPosHeight(vTemStart, h1);
...
void CollisionSupplierImp::GetTerrainInfo(const A3DVECTOR3& vPos, A3DVECTOR3& vPosOnSurface, A3DVECTOR3& vNormal)
{
	...
	vPosOnSurface=vPos;
	pTerr->GetPosHeight(vPos, vPosOnSurface.y, &vNormal);
