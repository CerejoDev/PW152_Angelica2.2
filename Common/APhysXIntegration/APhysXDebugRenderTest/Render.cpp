...
#define g_pA3DGDI A3DGDI::GetInstance()
///////////////////////////////////////////////////////////////////////////
//
...
bool CRender::InitA3DEngine()
{
	...
	if (!m_pA3DViewport->Init(g_Render.GetA3DDevice(), 0, 0, 
...
bool CRender::ResizeDevice(int iWid, int iHei)
{
	...
	m_pA3DEngine->SetDisplayMode(iWid, iHei, A3DFMT_UNKNOWN, 1, 1,  SDM_WIDTH | SDM_HEIGHT);
...
{
	if (m_pFont)
		m_pFont->AddText(x, y, szText, a_strlen(szText), color, 0, 0.0f, false);

	if(!m_pFont) return;

	int ih1 = m_pFont->GetFontHeight();
	int ih2 = m_pFont->GetFontOutputHeight();
}
