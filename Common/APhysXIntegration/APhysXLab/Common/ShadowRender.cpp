...
bool CECShadowRender::AllocResource()
{
	...
	m_pVertexShader = pA3DDevice->GetA3DEngine()->GetA3DShaderMan()->LoadVertexShader("Shaders\\vs\\shadow_vs.txt", false, dwVSDecl);
	...
	m_pShadowShader = pA3DDevice->GetA3DEngine()->GetA3DShaderMan()->LoadPixelShader("Shaders\\ps\\shadowrenderblur_14.txt", false);
...
bool CECShadowRender::Render(A3DViewport * pViewport, const A3DVECTOR3& vecLightDir)
{
	...
	for(int i=0; i<nNumShadow; i++)
	{
		...
		m_pShadowMapCamera->SetProjectionParam(-size * 3.0f, size * 3.0f, -size * 3.0f, size * 3.0f, -100.0f, 100.0f);
...
bool CECShadowRender::SetupStreamForShadower(A3DViewport * pViewport, const SHADOWER& shadower, const A3DVECTOR3& vecLightDir)
{
	...
	if( shadower.shadowReceiver & SHADOW_RECEIVER_TERRAIN )
	{
		...
		// get half size of the object
		float vSize = shadower.aabb.Extents.Magnitude();
		if( vSize > 6.0f )
			nx = nz = (int)vSize;
		...
		int alpha = min(255, max(0, (int)(255 * (1.0f - trace.fFraction + vSize / MAX_SHADOW_DIS))));
		...
		/////////////////////// DEBUG CODES To show shadow area
		//A3DWireCollector* pWireCollector = g_Render.GetA3DDevice()->GetA3DEngine()->GetA3DWireCollector();
		//A3DVECTOR3 v1, v2;
		...
		for(i=0; i<=nz; i++)
		{
			for(j=0; j<=nx; j++)
			{
				...
				////////////////
				//v1 = pVerts[nVertIndex].vPos;
				//v2 = v1 + A3DVECTOR3(0,1,0);
				//pWireCollector->Add3DLine(v1, v2, A3DCOLORRGBA(255, 255, 255, 255));
			}
		}
...
LITMODELPART:
	...
	{
	ECTriangleMan * pTriangleMan = g_pGame->GetTriangleMan();
		if( pTriangleMan )
		{
