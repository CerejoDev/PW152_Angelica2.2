...
CRender* CCoordinateDirection::g_Render = 0;
...
A3DVECTOR3 CCoordinateDirection::GetSelectedAxis()
{
...
void CCoordinateDirection::DrawAxis(A3DWireCollector& A3dWC) const
{
	...
	if(g_Render->GetA3DEngine()->GetActiveCamera()->GetWorldFrustum()->PointInFrustum(Vertices[1]))
		g_Render->TextOut(vScreenPos.x,vScreenPos.y,_AL("X"),A3DCOLORRGB(255,255,255));
	...
	g_Render->GetA3DEngine()->GetActiveViewport()->Transform(Vertices[1], vScreenPos);
	if(g_Render->GetA3DEngine()->GetActiveCamera()->GetWorldFrustum()->PointInFrustum(Vertices[1]))
		g_Render->TextOut(vScreenPos.x,vScreenPos.y,_AL("Y"),A3DCOLORRGB(255,255,255));
	...
	g_Render->GetA3DEngine()->GetActiveViewport()->Transform(Vertices[1], vScreenPos);
	if(g_Render->GetA3DEngine()->GetActiveCamera()->GetWorldFrustum()->PointInFrustum(Vertices[1]))
		g_Render->TextOut(vScreenPos.x,vScreenPos.y,_AL("Z"),A3DCOLORRGB(255,255,255));
}
...
//	ScreenTraceAxis(*g_Render->GetA3DEngine()->GetActiveViewport(), pt, pos, m_matRotate, &xy,&xz,&yz,dot);
	ScreenTraceAxis(*g_Render->GetA3DEngine()->GetActiveViewport(), pt, pos, mat, &xy,&xz,&yz,dot);
