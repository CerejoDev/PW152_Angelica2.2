...
public:
	...
	virtual void SkipMovingObjCollision(bool bSkip );
	
protected:
	//init param if needed
	bool Init();		
	
	virtual A3DTerrain2* GetTerrain();
	...
	virtual bool RayTraceEnv(const A3DVECTOR3& vStart,  const A3DVECTOR3& vDelta, A3DVECTOR3& vHitPos, A3DVECTOR3& vNormal, float& fraction);
	
	virtual bool CollideWithBrush(BrushCollideParam *pParam);

public:
	virtual float GetWaterHeight(const A3DVECTOR3& vPos);
...
bool ICollisionSupplier::VertTrace(BrushCollideParam *pParam, CDR_TRACE_OUTPUT* pOut, float DeltaY)
{
	...
	pOut->fFaction = -1.0f;	//	Set to invalid value
	pOut->iUserData1 = groundTrc.iUserData1;
	pOut->iUserData2 = groundTrc.iUserData2;
	
	pParam->vHitNormal = groundTrc.vHitNormal;
	pParam->fFraction = -1;
	pParam->iUserData1 = groundTrc.iUserData1;
	pParam->iUserData2 = groundTrc.iUserData2;
	return true;
} 

bool ICollisionSupplier::VertRayTrace(const A3DVECTOR3& vPos, A3DVECTOR3& vHitPos, A3DVECTOR3& vHitNormal, float DeltaY )
{
	A3DVECTOR3 vTerrainPos, vTerrainNormal, vBuildingPos, vBuildingNormal;
	float fraction;
	
	GetTerrainInfo(vPos, vTerrainPos, vTerrainNormal);
	if(RayTraceEnv(vPos, -DeltaY*A3DVECTOR3(0, 1, 0), vBuildingPos, vBuildingNormal, fraction))
	{
...
bool ICollisionSupplier::RetrieveSupportPlane(ground_trace_t * pTrc)
{
	...
	pTrc->iUserData1 = trcInfo.iUserData1;//碰撞的brushobj
	pTrc->iUserData2 = trcInfo.iUserData2;//sinked in brush
...
bool ICollisionSupplier::CollideWithEnv(env_trace_t * pEnvTrc)
{
	...
	pEnvTrc->dwClsFlag = 0;
	pEnvTrc->iUserData1 = pEnvTrc->iUserData2 = 0;
	if ((pEnvTrc->dwCheckFlag & CDR_BRUSH) == CDR_BRUSH)
	{
		...
		if (CollideWithBrush(&param))
		{
			...
			pEnvTrc->iUserData1 = param.iUserData1;//记录碰撞的brushobj
			pEnvTrc->iUserData2 = param.iUserData2;//sinked brush
...
bool ICollisionSupplier::CollideWithTerrain(const A3DVECTOR3& vStart, const A3DVECTOR3& vDelta, float& fFraction, A3DVECTOR3& vHitNormal, bool& bStart)
{
	...
	//						|
	A3DVECTOR3 vTempHitNormal,vTempEnd = vStart+vDelta, vTempDelta = vDelta,vTemStart = vStart;
	h1 = pTerrain->GetPosHeight(vTempEnd, &vTempHitNormal);
	...
	{	
		...
		do
		{
			...
			h1 = pTerrain->GetPosHeight(vTemStart, &vTempHitNormal);
...
{	
	...
	bool bClear = !CollideWithEnv(&trcInfo);
	pParam->iUserData1 = trcInfo.iUserData1;
	pParam->iUserData2 = trcInfo.iUserData2;
...
void CollisionSupplierImp::SkipMovingObjCollision(bool bSkip /* = true */)
{
	g_Game.GetBrushObjMgr().SkipMovingObjCollision(bSkip);
}

bool CollisionSupplierImp::Init()
{
	return true;
}
...
{
	//here we use capsule
	CBrushesObjMgr& objMgr = g_Game.GetBrushObjMgr();
	CCapsule capsule;
	capsule.SetRadius(pParam->fCapsuleR);
	capsule.SetHeight(pParam->fCapsuleHei);
	capsule.SetUp(pParam->vCapsuleUp);
	pParam->iUserData2 = pParam->iUserData1 = 0;
	
	A3DVECTOR3 normal;
	if (objMgr.CapsuleTrace(pParam->vStart, pParam->vDelta, pParam->fFraction, capsule, &normal, pParam->bStartSolid))
	{
		pParam->iUserData1 = (int)objMgr.m_CollidedBrush;//dwUser1保存碰撞到的BrushObj
		if (pParam->bStartSolid)
			pParam->iUserData2 = (int)objMgr.m_pSinkedBrush;//dwUser2保存陷入的Brush
...
bool CollisionSupplierImp::RayTraceEnv(const A3DVECTOR3& vStart,  const A3DVECTOR3& vDelta, A3DVECTOR3& vHitPos, A3DVECTOR3& vNormal, float& fraction)
{
	CBrushesObjMgr& objMgr = g_Game.GetBrushObjMgr();
	bool bStart = false;
	if (objMgr.RayTrace(vStart, vDelta, fraction, &vNormal, bStart))
	{
...
void CollisionSupplierImp::GetTerrainInfo(const A3DVECTOR3& vPos, A3DVECTOR3& vPosOnSurface, A3DVECTOR3& vNormal)
{
	...
	vPosOnSurface=vPos;
	vPosOnSurface.y = pTerr->GetPosHeight(vPos, &vNormal);
}
...
A3DTerrain2* CollisionSupplierImp::GetTerrain()
{
	return g_Game.GetTerrain();
}

float CollisionSupplierImp::GetWaterHeight(const A3DVECTOR3& vPos)
{
	//add code here
	return 0;
}
